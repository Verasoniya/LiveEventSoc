        ...
        <Layout>
          <div className="flex items-center my-10">
            <div className="grid grid-flow-row auto-rows-max w-full gap-8 my-8 mx-10 lg:mx-36 grid-cols-1 md:grid-cols-3 2xl:grid-cols-4 content-center">
              {allEvent.map((item) => (
                <CardHome key={item.id} imageEvent={item.image_url} titleEvent={item.event_name} dateStart={item.date_start} timeStart={item.start_at} price={item.price} onClickDetailEvent={() => router.push(`/detail-event/${item.id}`)} />
              ))}
            </div>
          </div>
        </Layout>
